...
- **SDK Session Pre-Warming** - On startup, all agents get fresh warm SDK sessions in parallel. First message always hits a warm session (~2-5s). SDK retry on error: if any session produces an error or 0-token empty response, the daemon closes the broken session, creates a fresh one, and retries on the SDK path. CLI subprocess is the absolute last resort.
- **Agent Self-Reflection** - After every non-trivial response, each agent self-assesses via a lightweight Haiku call: "Did I complete the task? Evidence? What's still open?" Reflections are stored and surfaced in the next heartbeat so agents catch their own gaps and follow up on incomplete work.
- **Delegation Verification Loop** - When an agent delegates work and receives a result, the calling agent is asked to verify: "Is this complete? Are there tests? Does it meet the acceptance criteria?" If not verified, the work is automatically re-delegated with specific feedback. Prevents "committed without tests" scenarios.
- **Proactive Initiative** - Every 15 minutes during idle periods (no user messages for 10 min), the orchestrator reviews pending tasks and open self-assessments, then autonomously dispatches work. Agents don't wait to be poked — they find and start their own work.
- **Conversation-Level Planning** - Multi-step delegations automatically create a plan object (persisted to SQLite) with steps and completion status. Active plans are injected into agent context so they track progress across messages: "Step 1 ✓, Step 2 in progress, Step 3 pending."
...
**Pre-warmed sessions.** On startup, all agents get fresh warm SDK sessions in parallel. The first message always hits a warm session (~2-5s). If any session errors or returns empty, the daemon retries on a fresh SDK session before falling back to CLI.
