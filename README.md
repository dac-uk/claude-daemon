...
- **Session Resume at Startup** - When the daemon restarts (manual, auto-update, crash), pre-warming attempts to resume each agent's most recent SDK session if `last_active` is within `sdk_resume_max_age_hours` (default 24h). Resume happens at startup — never on the user's first message — so failures are invisible. If the stored session has expired server-side, the fallback creates a blank session automatically. Users always hit an already-warm session on their first message.
- **SDK Retry on Error** - When any SDK session produces an error or a 0-token empty response, the daemon closes the broken session, creates a fresh one, and retries the message once — all on the SDK path. CLI subprocess is the absolute last resort. No nil responses, no silent empty replies.
...
**Session resume at startup.** When the daemon restarts, pre-warming attempts to resume each agent's most recent SDK session if `last_active` is within `sdk_resume_max_age_hours` (default 24h, set to 0 to disable). Resume happens during startup — never during a user message — so failures are invisible. If the server-side session has expired, a blank session is created instead. Either way, the first user message always hits a warm session with no delay or errors.
