...
class ClaudeDaemon:
    ...
    async def _precreate_agent_sessions(self) -> None:
        ...
        All sessions are created fresh (blank) — resume via unstable_v2_resumeSession
        proved unreliable (100% failure rate, error_during_execution on every attempt).
        Conversation continuity is handled by injecting recent history + semantic search.
        """
        ...
        for r in results:
            if isinstance(r, tuple):
                ...
                failed.append(f"<error: {r}>")

        log.info(
            "Pre-warmed %d/%d SDK sessions. OK: [%s]%s",
            len(succeeded), len(tasks),
            ", ".join(succeeded),
