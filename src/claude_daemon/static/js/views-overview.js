...
CC.renderOverviewMetrics = async function() {
  ...
  var cards = [
    ...
    { label: 'Sessions', value: s.total_sessions || 0, sub: s.active_sessions + ' active' + (s.warm_sessions ? ' · ' + s.warm_sessions + ' warm' : ''), clickable: true, clickAction: 'sessions' },
    { label: 'Discussions', value: dStats.total || 0, sub: (dStats.converged || 0) + ' converged' }
  ];

  el.innerHTML = cards.map(function(c) {
    var cls = 'metric-card glass-sm' + (c.clickable ? ' stat-clickable' : '');
    var attr = c.clickAction ? ' data-click="' + c.clickAction + '"' : '';
    return '<div class="' + cls + '"' + attr + '><div class="label">' + c.label + '</div><div class="value">' + c.value + '</div><div class="sub">' + (c.sub || '') + '</div></div>';
  }).join('');
  el.querySelectorAll('.metric-card[data-click]').forEach(function(card) {
    card.addEventListener('click', function() {
      var action = card.dataset.click;
      if (action === 'sessions') CC.openSessionsDrilldown();
    });
  });
};
